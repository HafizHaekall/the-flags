...
const drop = (e) => {
  ...
  if (isTouchDevice()) {
    ...
    if (draggedElementData === droppableElementData) {
      ...
      e.target.insertAdjacentHTML(
        "afterbegin",
        `<img src="assets/${draggedElementData}.png" id="${draggedElementData}">`
      );
      count += 1;
    }
  }
  //Win
  if (count == 3) {
    result.innerText = `Berhasil!`;
    stopGame();
    // Ubah teks tombol "Mulai" menjadi "Lanjut"
    startButton.innerText = "Lanjut";
    const spanElement = document.getElementById("develop");
...
const creator = () => {
  ...
  for (let i of randomData) {
    ...
    flagDiv.innerHTML = `<img src="assets/${i}.png" id="${i}">`;
